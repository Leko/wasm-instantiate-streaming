...
const suite = (instantiateStreaming, loader) => {
  it("returns object that contains module", () => {
    return instantiateStreaming(loader("no-deps.wasm")).then(result => {
  ...
  it("can pass importObject", () => {
    ...
    return instantiateStreaming(loader("with-deps.wasm"), imports).then(
      result => {
        assert.notEqual(typeof result.instance, "undefined");
      }
    );
  ...
  it("can add works", () => {
    return instantiateStreaming(loader("no-deps.wasm")).then(({ instance }) => {
