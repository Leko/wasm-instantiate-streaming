...
if (typeof WebAssembly !== "undefined") {
  describe("instantiateStreaming", () => {
    describe("node-fetch", () => {
      ...
      suite(instantiateStreaming, wasmName =>
        fetch(`http://localhost:${PORT}/${wasmName}`)
      );
    });
    ...
    describe("fs", () => {
      ...
      suite(instantiateStreaming, wasmName =>
        readFile(path.join(__dirname, "..", "fixtures", wasmName))
      );
    });
  });
} else {
  describe("instantiateStreaming (in environment that WebAssembly is not supported)", () => {
    it("must reject Promise", () => {
      return instantiateStreaming()
        .then(() =>
          assert.fail(
            "must must be reject instantiateStreaming in environment that WebAssembly is not supported"
          )
        )
